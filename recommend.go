    ...
    Follow_user_id := map[uint32]bool {}
    Follow_user_id0 ,ok := Follow_user.Get(openid) //Openid_like_list[openid]
    if ok {
        Follow_user_id_t := Follow_user_id0.([]uint32)
        for _,v := range Follow_user_id_t {
            Follow_user_id[v] = true
        }
    }

    fmt.Println("Follow_user_id len:",len(Follow_user_id))
    ...
    if(cmd == "new"){
        ...
        get_follow_video_n:=0
        //check if the followed user post
        if len(Follow_user_id)>0{
            for short_id := uint32(0) ;short_id<all_len && short_id<10000; short_id++ {
                my_user_upvideo_lock.RLock()
                var my_user_upvideo_t = my_user_upvideo[short_id]
                my_user_upvideo_lock.RUnlock()
                vid_1 := my_user_upvideo_t.vid

                g.Video_data_rwmutex.RLock()
                video_info,ok := g.Video_data[vid_1]
                g.Video_data_rwmutex.RUnlock()
                
                if ok  {
                    _,ok2 := Follow_user_id[video_info.Uid]
                    if ok2 {
        ...
        for short_id := uint32(0) ;short_id<all_len ; short_id++ {
            ...
            if((!isset ||  Recommend_param[18] ==0) && vid_1>0 ){
                ...
                // ,search_result
                if int64(count + get_follow_video_n) >= limit1 + limit2{
                    break
                } 
            }
        }
        ...
        for i ,id1 := range find_ids {
            ...
            for j:=0;j<5;j++{
                ...
                //去除我收藏列表的 几级推荐视频 （可能在抖音上已看过）
                if Recommend_param[11]!=0 && Recommend_param[11]<5 {
